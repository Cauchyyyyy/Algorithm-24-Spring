...
class GraphCut(object):
    ...
    def __init__(self,pattern,rows,cols):
        ...
        self.rowImport = rows
        self.colImport = cols
        self.old = np.zeros((rows,cols,3),dtype = int)
        self.new = np.zeros((rows,cols,3),dtype = int)
        self.mask = np.zeros((rows,cols),dtype = int)
        self.overlap_zone = np.zeros((rows,cols),dtype = int)
        self.seams = np.zeros((rows,cols,2),dtype = int)
        self.init_value_seams = np.zeros((rows,cols,2),dtype = int)
        self.maxp = self.rowPatch * 30
        self.minp = self.colPatch * 14
        self.masking_edge = [self.rowImport,0,self.rowImport,0]
        self.index = 1
        self.temp = np.zeros((rows,cols,3),dtype = int)
    

    def change_masking(self,t):
        maxi = min(t[0] + self.rowPatch, self.rowImport)
        maxj = min(t[1] + self.colPatch, self.colImport)
        self.mask[t[0]:maxi,t[1]:maxj] = np.ones((maxi-t[0],maxj-t[1]),dtype=int)
    ...
    def change_overlap(self,t):
        self.overlap_zone = np.zeros((self.rowImport,self.colImport),dtype=int)
        edge_point = [0,0]
    ...
    def patching_placement(self):
        ...
        patching = np.zeros((self.realRows,self.realCols),dtype = float)
        tk = np.zeros((self.realCols, self.rowPatch, self.colPatch, 3),dtype = float)
    ...
    def minCut_calculate(self,t):
        ...
        mask_seam = np.zeros((self.rowOverlap, self.colOverlap ),dtype = int)
        num = 2
        seam_supp = 0
        mat_num = np.zeros((self.rowOverlap, self.colOverlap ),dtype = int)
        ...
        # file.close()
        print('mincut calculating... ',datetime.now())
        # print(krt, partition)
        ...
        self.overlap_zone = np.zeros((self.rowImport,self.colImport),dtype=int)
        self.index += 1
        self.temp = self.new[0:self.realRows,0:self.realCols]
        self.temp=self.temp.astype('uint8')
        cv2.imshow('output',self.temp)
        cv2.waitKey(100)
    ...
    def patching_func(self):
        ...
        while(y < self.realCols):
            ...
            x = 0
        print('mask has been filled, start to "Optimise"\n')
        for i in range(5):
            #print(i)
...
if __name__ == '__main__':
    ...
    if len(sys.argv)==5:
        filename = sys.argv[1]
        filename_final = sys.argv[2]
        width_final = int(sys.argv[3])
        height_final = int(sys.argv[4])
    else:
        print('no parameters accepted, use the default set\n')
        filename = 'green.png'
        filename_final = 'result.jpg'
        width_final = int(256)
        height_final = int(256)
    ...
    cv2.waitKey(100)
    a = np.array(img,dtype=int)[:,:,0:3]
    print('graphcut begin...',datetime.now())
    graphcut = GraphCut(a,width_final,height_final)
    ...
    cv2.imwrite(filename_final,rst)
    print('Done! The result has been saved as ',filename_final)